...
figure()
semilogy(Ecut_list, [diff_list[i,i] for i in 1:length(Ecut_list)] / norm(f_ref), label="F-F*")
semilogy(Ecut_list, [diff_newton_list[i,i] for i in 1:length(Ecut_list)] / norm(f_ref), label="Fnewton-F*")
semilogy(Ecut_list, [diff_list_err[i,i] for i in 1:length(Ecut_list)] / norm(f_ref), label="F-Ferr-F*")
semilogy(Ecut_list, [diff_list_res[i,i] for i in 1:length(Ecut_list)] / norm(f_ref), label="F-Fres-F*")
semilogy(Ecut_list, [diff_list_schur[i,i] for i in 1:length(Ecut_list)] / norm(f_ref), label="F-Fschur-F*")
legend()
